...
function renderHeader(){
 ...
 const heroUser=$("#heroUser");if(heroUser)heroUser.textContent=currentUser||"尚未設定";
 ...
 const changeButton=$("#heroChangeUser");if(changeButton)changeButton.onclick=openUser;
}
...
function renderDashboard(){
 ...
 const weddingLabel=weddingDate
   ? new Date(`${weddingDate}T00:00:00`).toLocaleDateString("zh-TW",{month:"numeric",day:"numeric",weekday:"short"})
   : "婚禮當天";
 ...
 const nextFlow=isWeddingDay
   ? relatedFlows.map(f=>({f,min:minutesFromNowForFlow(f)}))
       .filter(x=>x.min!==null&&x.min>=0)
       .sort((a,b)=>a.min-b.min)[0]?.f
   : null;
 ...
 <section class="wedding-day-card card">
  <div class="wedding-day-heading">
    <div>
      ...
      <div class="meta">只顯示婚禮當天${weddingDate?`（${esc(weddingDate.replaceAll("-","/"))}）`:""}</div>
    </div>
    <span class="date-pill">${esc(weddingLabel)}</span>
  </div>

  ${nextFlow?`<div class="next-mini-card">
    <div class="next-mini-label">下一個行程</div>
    <div class="next-mini-time">${esc(formatFlowTime(nextFlow)||"未定")}</div>
    <div class="next-mini-title">${esc(nextFlow.icon||"📍")} ${esc(nextFlow.name)}</div>
    ${nextFlow.location?`<div class="next-mini-location">📍 ${esc(nextFlow.location)}</div>`:""}
    ${mapUrl(nextFlow)?`<a class="map-link" href="${esc(mapUrl(nextFlow))}" target="_blank" rel="noopener">開啟導航</a>`:""}
  </div>`:`<div class="next-mini-card muted-next">
    <div class="next-mini-label">下一個行程</div>
    <div class="next-mini-title">${isWeddingDay?"目前沒有接下來的行程":"婚禮當天才會顯示即時下一個行程"}</div>
  </div>`}
  ...
  <div class="day-timeline">
   ...
   </div>`).join("")||'<div class="empty">目前使用者尚未有婚禮當天行程</div>'}
  </div>
 </section>
 ...
 <section class="dashboard-shortcuts">
   ...
   <button class="shortcut-card" data-jump="prepare">
     <span class="shortcut-icon orange">💼</span>
     <strong>婚禮準備</strong>
     <em>${tasks.filter(x=>!x.done).length} 項未完成</em>
   </button>
 ...
 <section class="card compact-detail-card">
  ...
  ${workEntries.map(x=>{
    ...
    return `<div class="row">
      ...
      <div class="main"><div class="name">${esc(x.title)}</div><div class="meta">${esc(x.meta)}</div></div>
      ${x.flow?`<div class="actions"><button class="small" data-action="go-flow" data-id="${x.flow.id}">查看流程</button></div>`:""}
    </div>`;
